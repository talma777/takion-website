(function ($) {
	...
	if($('.active-sticky-header').length){
		...
		const headerHeight = headerSticky.outerHeight(); // Altura inicial

		function setHeaderHeight(){
			$("header.main-header").css("height", headerHeight); // Usamos la altura inicial
		}

		setHeaderHeight(); // Configurar altura al cargar

		$window.on("scroll", function() {
			const fromTop = $(window).scrollTop();
			
			// Lógica de ocultar/mostrar
			if (fromTop > lastScroll && fromTop > headerHeight) {
				headerSticky.addClass("hide").removeClass("active");
			} else {
				headerSticky.removeClass("hide").addClass("active");
			}
			
			// Resetear si está en top
			if (fromTop < 100) {
				headerSticky.removeClass("active hide");
			}
